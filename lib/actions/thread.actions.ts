...
export async function likeThread(threadId: string, userId: string) {
  try {
    ...
    if (!thread) {
      // Handle scenario where the thread is not found
      throw new Error("Thread not found");
    }

    const alreadyLiked = thread.likes.includes(user._id);
    ...
    if (alreadyLiked) {
      ...
      // Add the liked thread to the user's likedThreads array
      await User.findOneAndUpdate(
        { id: userId },
        {
          $push: { likedThreads: thread._id },
        }
      );
    }
